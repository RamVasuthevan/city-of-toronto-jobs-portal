...
class DownloadSearchPageProvider(SearchPageProvider):
    ...
    def search_page(portal: Portal, page_number: int) -> HTMLString:
        ...
        try:
            ...
            if 'text/html' not in response.headers.get('Content-Type', ''):
                raise DownloadError(f"Expected HTML content but received '{response.headers.get('Content-Type')}' on page {page_number}")
    ...
    def search_pages_for_portals(portals: List[Portal] = None) -> Dict[Portal, List[HTMLString]]:
        ...
        return {
            portal: DownloadSearchPageProvider.search_pages_for_portal(portal)
...
class DirectorySearchPageProvider(SearchPageProvider):
    ...
    def search_pages_for_portals(portals: List[Portal] = None) -> Dict[Portal, List[HTMLString]]:
        ...
        return {
            portal: DirectorySearchPageProvider.search_pages_for_portal(portal)
            for portal in portals
        }